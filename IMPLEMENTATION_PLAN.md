...
### Phase 13 — GST / tax rules (India — deep analysis; admin & retailer both)
**Goal:** make every price the system quotes **GST-aware and legally coherent** for an Indian wholesale trader. The admin configures *what tax applies* (rate per product/HSN, and whether stored prices are tax-**inclusive** or -**exclusive**); the retailer sees prices and cart/order totals with a correct **CGST+SGST / IGST** breakup. Because there is **no payment** (Phase 12 is a purchase-request flow), what we produce is an **indicative proforma / quotation with GST**, computed exactly — *not* a statutory tax invoice (that is raised offline on actual supply). This keeps us out of e-invoicing/IRN/e-way-bill territory while still showing the trader and their buyer the real landed number.

**Grounding in the current model (do NOT re-derive):** all money is integer **paise** (`src/lib/money.ts` — `formatPaise`, `assertPaise`, `adjustPaise`; never floats). `Product`/`ProductVariant` today have `price`/`mrp` (paise) and **no** tax fields; `Category` has **no** tax default; `Customer.gstNumber` exists but is unparsed; `Order` snapshots `items`(Json)+`subtotalPaise`+`itemCount` with **no** tax; there is **no** global settings model yet. The price gate is a data-layer allow-list: `PublicProduct` has *no* money fields, `PricedProduct` adds them. **Every new monetary tax figure lives on the Priced projection only.**

**Two orthogonal axes (keep them separate — the usual source of GST bugs):**
1. **Storage/entry treatment** — does the stored `price`/`mrp` paise *already include* GST (**inclusive**, MRP-style) or is it the **taxable base** GST is added onto (**exclusive**)? This is a property of the *data*.
2. **Display treatment** — how the retailer is *shown* the number (incl-GST landed price vs base "+ GST"). A viewer preference, independent of storage; an inclusive-stored price can still be shown broken-down and vice-versa.
Conflating these two is the classic mistake; the design treats them as independent settings and always computes the breakup from the stored treatment.

**Data model (schema + migration — done by the driver, never a workflow):**
- **`SellerTaxProfile` (singleton settings doc)** — `{ gstin, legalName, stateCode (2-digit place-of-supply origin), priceEntryMode: TAX_EXCLUSIVE|TAX_INCLUSIVE (global default), displayMode: INCLUSIVE|EXCLUSIVE, roundingMode: LINE|INVOICE, defaultGstRateBps: Int, pricesTaxInclusiveDefault: Boolean, gstEnabled: Boolean }`. `gstEnabled=false` ⇒ the whole feature is inert and the app behaves exactly as today (safe rollout / non-GST sellers).
- **`Product` / `ProductVariant`** — add `hsnCode: String?`, `gstRateBps: Int?` (**basis points**, `1800` = 18.00% — supports 0/5/12/18/28 and fractional), `taxTreatment: TAX_EXCLUSIVE|TAX_INCLUSIVE|null` (per-product override; null ⇒ inherit profile). Variant inherits the product's HSN/rate/treatment unless it sets its own (accessories rarely differ, but power-bank vs cable can).
- **`Category`** — add `defaultHsnCode: String?`, `defaultGstRateBps: Int?` (inheritance: product → category → profile default).
- **`Customer`** — keep `gstNumber`; add `gstStateCode: String?` (first 2 digits of a validated GSTIN) and `placeOfSupplyStateCode: String?` (explicit billing state; GSTIN state wins for registered buyers). Validate GSTIN: 15-char format + checksum; unregistered buyers (no GSTIN) are allowed — place of supply comes from billing state.
- **`Order` (extend the snapshot — rates change, so the order must freeze them):** per line add `taxableValuePaise, gstRateBps, hsnCode, taxAmountPaise, cgstPaise, sgstPaise, igstPaise`; order-level add `totalTaxablePaise, totalCgstPaise, totalSgstPaise, totalIgstPaise, totalTaxPaise, roundOffPaise, grandTotalPaise, supplyType: INTRA|INTER, placeOfSupplyStateCode, sellerGstin, sellerStateCode`, plus an **HSN summary** array (group by `hsnCode`+`gstRateBps`: taxable, tax). Snapshot the profile's mode + seller identity too, so a placed quote is fully self-describing even if settings later change.

**Tax computation core — paise-exact, pure, server-authoritative (`src/lib/gst.ts`, unit-tested):**
- **Exclusive:** `taxable = base`; `tax = round(base × bps / 10000)`; `gross = base + tax`.
- **Inclusive:** `gross = stored`; `taxable = round(gross × 10000 / (10000 + bps))`; `tax = gross − taxable` (tax is the **remainder**, never independently rounded ⇒ `taxable + tax === gross` always holds — no drift).
- **Intra vs inter-state** (decided server-side): `seller.stateCode === place-of-supply` ⇒ **CGST + SGST**, split `cgst = floor(tax/2)`, `sgst = tax − cgst` (remainder to SGST ⇒ no 1-paise leak); else ⇒ **IGST = tax**.
- **Rounding policy:** GST invoices round **per line**; optional invoice-level round-off to the nearest rupee emitted as an explicit `roundOffPaise` line (never silently absorbed). Configurable via `roundingMode`.
- **Mixed-rate cart:** totals are summed per line then grouped into the HSN summary (per rate) — standard invoice layout. Zero-rated/exempt items (`gstRateBps = 0`) and `price = 0` are handled (tax 0, still listed).
- Reuse `assertPaise`; integer-only throughout; **no floats ever touch a stored amount.**

**Price-gate interaction (SACRED — do not weaken):** a GST *amount* (taxable/tax/CGST/SGST/IGST/gross paise) **is a price** ⇒ it is gated exactly like `price`. Only `PricedProduct`/`PricedVariant` and an **approved** customer's cart/order may carry tax paise. `hsnCode`, `gstRateBps`, and the `taxInclusive` boolean are **non-monetary public metadata** (HSN + rate% are printed on the box / statutory-public) and *may* appear on `PublicProduct` to render the label "incl./+ 18% GST" — but **no computed paise**. The DAL `select` allow-list enforces this; an adversarial test asserts **no tax-amount field appears in any gated (`PublicProduct`/anon/pending/expired) payload**, extending the existing zero-price-leak suite.

**Admin rules & UI (per §0 — custom components, all states):**
- **Settings → Tax:** seller GSTIN (validated) + state, `gstEnabled`, global entry mode (incl/excl), display mode, rounding mode, default rate. Clear copy on inclusive vs exclusive with a live worked example.
- **Category:** default HSN + rate. **Product/variant editor & DealSheet grid:** HSN, rate, treatment override — bulk-settable in the grid (reuses the series-fill/command stack) and via **CSV import columns** (`hsn_code`, `gst_rate`, `tax_inclusive`); **CSV export** gains the same columns and the per-order tax breakup.
- **Admin order/quote view:** full per-line + summary breakup, HSN summary table, supply-type (intra/inter) and both parties' GSTIN/state, round-off line, grand total.

**Retailer rules & UI:**
- **Listing / card / product page:** price rendered with its treatment — "₹499 **incl.** 18% GST" or "₹499 **+** 18% GST" — consistently; a **global toggle** (persisted preference) to view all prices incl/excl GST. Gated viewers see the locked chip exactly as today (no tax numbers).
- **Cart:** per-line taxable + GST, then a **tax summary block** — CGST+SGST *or* IGST depending on the customer's place of supply vs seller state — subtotal (taxable), total tax, round-off, **grand total**. If the customer has no GSTIN/state yet, show a **combined "GST @X%"** with an inline prompt to add GSTIN/billing state to see the CGST/SGST split (never guess the split).
- **Order confirmation + history + reorder:** show the frozen proforma breakup; label it **"Proforma / Quotation — not a tax invoice"** with the "we'll confirm & raise the invoice on dispatch" copy from Phase 12.

**Corner cases (handled explicitly):** rate changed after add-to-cart (cart shows live rate; order snapshots at placement — later catalog change never mutates a placed order); mixed inclusive/exclusive products in one cart (each line computed by its own treatment); customer GSTIN state ≠ billing state (registered ⇒ GSTIN state wins); unregistered buyer (billing state drives place of supply; still charged GST, no split ambiguity); exempt/zero-rated (`0 bps`) and free (`0 paise`) lines; variant without its own rate (inherit product); `gstEnabled=false` (feature inert, legacy behaviour); MRP defined inclusive while price exclusive (each field carries its own interpretation from treatment); very large mixed carts (rounding drift proven bounded by the remainder-split rules).

**Non-goals (explicit — this is a catalogue quote, not accounting):** payment/collection, statutory **tax invoice** issuance, **e-invoicing/IRN**, **e-way bill**, GSTR returns/filing, TCS/TDS, **compensation cess**, **reverse charge (RCM)**, SEZ/export/zero-rated *supply* handling, **composition scheme**, multi-GSTIN sellers. We compute a correct **indicative proforma with GST**; the real invoice is raised in the trader's accounting software on actual supply.

**Gate:** paise-exact unit suite green (inclusive↔exclusive round-trip; `taxable+tax===gross`; CGST/SGST no-drift split; IGST path; mixed-rate + HSN-summary totals; invoice round-off); intra vs inter-state selection correct by seller/customer state; **placed order freezes the rate** (later catalog rate change leaves the order unchanged) — an explicit test; GSTIN format/checksum validation; **price-gate adversarial test — zero tax-amount leak in any gated payload** (extends the §5 suite); `gstEnabled=false` reproduces exact legacy behaviour; typecheck+lint+vitest green.
**Sequencing:** its own schema step (SellerTaxProfile singleton + Product/Category/Variant tax fields + Order tax snapshot + Customer state), then a build workflow. Depends on **Phase 2** (price gate — tax amounts are gated), **Phase 12** (Orders — the snapshot lives here), and rides on **Phase 11** (variant inheritance). Uses `money.ts`. Not required for first launch, but expected before real B2B invoicing conversations.
...
| 13 GST/tax | M | paise-exact tax core + gated breakup; schema step + build |
