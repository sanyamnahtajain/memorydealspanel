...
describe("paste mode", () => {
  it("fills the rows from a pasted list and reports unmatched lines plainly", async () => {
    ...
    expect(
      await screen.findByText("Could not find: Nokia 3310"),
    ).toBeInTheDocument();
    expect(screen.getByText("Filled 2 models.")).toBeInTheDocument();
    // Existing row's qty replaced; the new model appended.
    expect(onChange).toHaveBeenCalledWith([
      { ...S23, qty: 50 },
      { modelId: "c", name: "Redmi Note 13", qty: 30 },
    ]);
